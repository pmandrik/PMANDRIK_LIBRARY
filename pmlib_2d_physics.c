...
#include "pmlib_const.hh" // pm::PI_180 = 3.141 / 180.
...
#define DPRINT(x) do{}while(0)
  ...
  class pmTiledMap{
    ...
    void SetMapTypeSafe(const int & x, const int & y, const float * map_data){
      ...
      std::memcpy(mdata + msize_types_x_msize_y*x + msize_types*y, map_data, msize_types * sizeof(int));
    }
        ...
        */
        ...
        if( obj_grid.find(bkey) != obj_grid.end() ){
          ...
          for(unsigned int id_index = 0; id_index != ids.size(); id_index++){
            ...
            // => return ids of objects
            interacted_bullet_x_object_pairs.push_back( std::make_pair<int, int>(int(i_2), int(obj_id/2)) );
          }
        }
